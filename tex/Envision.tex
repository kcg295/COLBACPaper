...
authorized to perform an action. This decision as to whether or not a person is
authorized is heavily informed by an organization's security policy.

However, as it is natural with the notion of authorization, two fundamental
pieces must exist: authorization \emph{policies} that represent
collective-based authorization, and an access control system that can
\emph{enforce} such policies.
...
Access control is commonly encoded and enforced in a top-down fashion. The CISO
of an organization holds institutional power to define access control policies
within an orgnization. Further, without loss of generallity, lower-level
administrators may have some decision making power in how to define subsets of
a policy, yet, they typically do not have decision making power to. 
